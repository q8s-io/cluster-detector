...
import (
	...
	pod_core "github.com/q8s-io/cluster-detector/pkg/core"
...
var timeoutThreshold = configs.Config.PodInspectionConfig.TimeoutThreshold
...
type PodInspectionSource struct {
	...
	localPodBuffer chan *pod_core.PodInspection
	// count pod num
	num         int
...
func (this *PodInspectionSource) GetNewPodInspection() *pod_core.PodInspectionBatch {
	result := pod_core.PodInspectionBatch{
		TimeStamp:   time.Now(),
		Inspections: []*pod_core.PodInspection{},
	}
...
func (this *PodInspectionSource) filter(pod *v1.Pod) *pod_core.PodInspection {
	...
	podInspection := new(pod_core.PodInspection)
...
func NewPodInspectionSource(uri *url.URL) (*PodInspectionSource, error) {
	...
	result := PodInspectionSource{
		localPodBuffer: make(chan *pod_core.PodInspection, LocalPodsSize),
