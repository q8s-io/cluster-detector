...
type realManager struct {
	source    core.PodInspectionSource
	sink      core.PodSink
	frequency time.Duration
	stopChan  chan struct{}
}

func NewManager(source core.PodInspectionSource, sink core.PodSink, frequency time.Duration) (manager.Manager, error) {
	manager := realManager{
		source:    source,
		sink:      sink,
		frequency: frequency,
...
func (rm *realManager) Housekeep() {
	times := configs.Config.PodInspectionConfig.Speed
	if times == 0 {
